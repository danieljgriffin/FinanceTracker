...
Animation preferences: User wants animated number cycling on net worth updates with smooth rolling transition effects.
...
- **Trading 212 API Integration**: Automated portfolio synchronization with Trading 212 using Basic Authentication (API_KEY + API_SECRET). Features daily scheduled sync to avoid rate limits (50 requests/minute), intelligent currency conversion (USD→GBP for US stocks, pence→pounds for UK stocks), and automatic duplicate detection. API serves as single source of truth for Trading 212 holdings.
